...
/*   Updated: 2022/05/27 23:27:36 by vcordeir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cast_rays(t_game_set *game_set)
{
	...
	strip_id = 0;
	distance_projected = (game_set->window_width / 2) / tan(field_of_view / 2);
	while (strip_id < game_set->number_of_rays)
	{
		ray_angle = (game_set->player)->rotation_angle + \
			atan((strip_id - game_set->number_of_rays / 2) / \
			distance_projected);
